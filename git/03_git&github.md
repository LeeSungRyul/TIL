...
- `git bash`에서 `TIL` 폴더로 이동한 이우에 아래의 명령어로 `git` 관리를 시작한다.
...
- `git` 설정 (user.name & user.email) : **최초 1회 설정**
  ...
  ```shell
  $ git config --global user.name "SungRyul"
  $ git config --global user.email "airtrack03@naver.com"
...
- `README.md` 파일을 `TIL` 폴더(최상단)에 생성한다. 이름은 반드시 **README.me**로 설정한다.
